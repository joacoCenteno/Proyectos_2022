...
<body>
    <nav class="nav">
        ...
        <div class="turno_contenedor">
            <a href="index.php" class="link_turno">Cerrar Sesión</a>
        </div>
    </nav>
    ...
    <?php
            ...
            $sql="SELECT `nombre`, `banner` FROM `muestras` WHERE `id_muestra`=3;"; //Se realiza el SQL de tipo SELECT y se seleccionan los campos nombre y banner de la tabla muestras cuando el id_muestra sea 3
    ...
    <section class="sound">
        <h3 class="tile_sound">¿Quieres dar a conocer a personas con dificultades visuales sobre lo que se ofrece en las exposiciones de Geología?</h3>
    ...
    <main class="main">
        <h2 class="title">Visitanos Y Descubre Las Maravillas De Este Mundo</h2>
        ...
        <?php
            ...
            if(isset($_POST['btnGuardar'])){ //Se verifica que exista la vatiable POST con el name btnGuardar
                ...
                $firstsql ="UPDATE `muestras` SET `descripcion`='$newText' WHERE `id_muestra` = 3;"; //Se realiza la primera consulta SQl UPDATE y se actualiza el campo descripcion de la tabla muestras cuando el id_muestra sea 3 (Geologia)
                $firstresult = mysqli_query($con,$firstsql); //Se almacena el resultado de la consulta en la variable $firstresult
            }

            $sql="SELECT `descripcion`, `img` FROM `muestras` WHERE `id_muestra`=3;"; //Se realiza la segunda consulta SQL, en este caso un SELECT y se selecciona los campos descripcion e img de la tabla muestras cuanod el  id_muestra sea 3 (Geologia)
            $result = mysqli_query($con,$sql); //Se almacena el resultado de la consulta en la variable $result
    ...
    <script>
       let button = document.querySelector('.button_sound');  //Se crea la variable button y se le almacena el elemento con la clase button_sound
        ...
        var audio = new Audio('sounds/geologia.mp3');
        audio.play();
        })
