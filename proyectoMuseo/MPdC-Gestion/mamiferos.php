...
<body>
    ...
    <section class="sound">
        <h3 class="tile_sound">¿Quieres dar a conocer a personas con dificultades visuales sobre lo que se ofrece en las exposiciones de Mamíferos?</h3>
    ...
    <main class="main">
        <h2 class="title">Una Excelente Opción Para Todos</h2>
        ...
        <?php
            ...
            if(isset($_POST['btnGuardar'])){ //Se verifica que exista la vatiable POST con el name btnGuardar
                ...
                $firstsql ="UPDATE `muestras` SET `descripcion`='$newText' WHERE `id_muestra` = 3;"; //Se realiza la primera consulta SQl UPDATE y se actualiza el campo descripcion de la tabla muestras cuando el id_muestra sea 2 (Mamiferos)
                $firstresult = mysqli_query($con,$firstsql); //Se almacena el resultado de la consulta en la variable $firstresult
            }

            $sql="SELECT `descripcion`, `img` FROM `muestras` WHERE `id_muestra`=3;"; //Se realiza la segunda consulta SQL, en este caso un SELECT y se selecciona los campos descripcion e img de la tabla muestras cuanod el  id_muestra sea 2 (Mamiferos)
            $result = mysqli_query($con,$sql); //Se almacena el resultado de la consulta en la variable $result
    ...
    <script>
        ...
        var audio = new Audio('sounds/mamiferos.mp3');
        audio.play();
        })
